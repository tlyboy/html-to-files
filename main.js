...
mkdir(new URL('./dist', import.meta.url), err => {
  if (err) return
})

// 2.1 读取需要被处理的 HTML 文件
readFile(new URL('./src/index.html', import.meta.url), 'utf8', (err, dataStr) => {
  // 2.2 读取 HTML 文件失败
...
function resolveCSS(htmlStr) {
  ...
  // 3.4 将提取出来的 css 样式，写入到 style.css 文件中
  writeFile(new URL('./dist/style.css', import.meta.url), newCSS, err => {
...
function resolveJS(htmlStr) {
  ...
  // 4.4 将提取出来的 js 脚本，写入到 main.js 文件中
  writeFile(new URL('./dist/main.js', import.meta.url), newJS, err => {
...
function resolveHTML(htmlStr) {
  ...
  const newHTML = htmlStr
    ...
    .replace(regScript, '<script src="./main.js"></script>')
  // 5.3 将替换完之后的 html 代码，写入到 index.html 文件中
  writeFile(new URL('./dist/index.html', import.meta.url), newHTML, err => {
